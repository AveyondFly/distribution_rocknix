...
**RK3326 当前仅提供这一套通用镜像**（主线习惯称 **image-b** 或与 RK3326 设备树对应的单个构建产物），下列 **全部机型均由该镜像覆盖**——刷入后按设备 quirks／首次引导选择或自动识别机型即可；若你的机器外观名称在表中但与当前硬件改版不兼容，请到社区核对 PCB／屏幕批次。
...
### 8.1 `rocknix2emmc.sh`：TF 克隆到 eMMC，无卡启动

适用于 **RK3326** 等平台、**板载 eMMC 且当前从 TF（SD）启动**的克隆／寨机（例如 **GameConsole** 系列 **R36S** 等一批带 eMMC 的机型）。

- **作用**：从 **TF 卡**（脚本中假定块设备 **`/dev/mmcblk1`**）把 **引导与 ROCKNIX FAT 分区**等内容克隆到 **eMMC**（**`/dev/mmcblk0`**），并处理分区 UUID 与签名以避免与 TF 冲突，在 eMMC 上生成可用的 **storage** 分区思路与主线「装到内置」一类流程一致（具体分区布局以脚本与日志为准）。
