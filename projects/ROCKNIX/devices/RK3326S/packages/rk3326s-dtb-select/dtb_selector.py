...
def _remount_flash(rw: bool) -> None:
    if os.environ.get("RK3326S_DTB_SELECT_NO_REMOUNT"):
        return
...
# RK3326S：单层列表（与 config.xml 中 RK3326S 的 DTB 一致）
ALL_DEVICES: List[Tuple[str, str]] = [
    ("漫特科技 GameMT E6", "rk3326s-gamemt-e6.dtb"),
    ("GameKiddy GKD Pixel2", "rk3326s-gkd-pixel2.dtb"),
]
...
def apply_fdt_to_extlinux(dtb_rel: str, *, no_color=False) -> bool:
    ...
    remount = base == "/flash" and not os.environ.get("RK3326S_DTB_SELECT_NO_REMOUNT")
    if remount:
        _remount_flash(True)
...
def main():
    ...
    while True:
        clear_screen()
        print_header("RK3326S — 请选择机型", no_color=False)
